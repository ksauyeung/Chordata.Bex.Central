...
{
    public class MakerTakerLeft : Interface.IStrategy
    {
        ...
        //hurdle is boosted for a certain period if order is cancelled due to post-only
        ...
        public void Start()
        {
            ...
            Message("Running Maker-Taker-Left..." + System.Threading.Thread.CurrentThread.ManagedThreadId.ToString(), 0);
            Run();
        }
        ...
        private async void Run(int OpID = -1)
        {
            ...
            using (var db = new Tuna())
            {
                log.Debug(db.Database.Exists().ToString());

                //OPs left join Runs
                var queryOpRuns = from op in db.OPs.Where(o => o.Name == "Maker-Taker-Left" && o.Enabled == true && (OpID == -1 || o.Id == OpID))
        ...
        private async Task<IResult> ClosePosition(MakerTakerSet set, string reason)
        {
            ...
            if (!string.IsNullOrEmpty(record.Order1) && !string.IsNullOrEmpty(record.Order2)) //case 1
            {
                ...
                {
                    //IResult res = await OrderManager.Instance.CancelOrderAsync(set.Maker, record.Order3);
            ...
            else
            {
                ...
                {
                    IResult res = await OrderManager.Instance.CancelOrderAsync(set.Maker, record.Order1);
                    if (!res.Success)
        ...
        private void AfterPositionClosed(MakerTakerSet set)
        {
            ...
            using (var db = new Tuna())
            {
                ...
                Message("Saving..." + set.RunRecord.Id.ToString(), set.RunRecord.Id);
                db.SaveChanges();
                Message("Saved." + set.RunRecord.Id.ToString(), set.RunRecord.Id);
            }
            makerTakerSets.Remove(set.RunRecord.Id);
        ...
        private async Task<bool> PrepareOrders(MakerTakerSet set)
        {
            ...
            else if (string.IsNullOrEmpty(set.RunRecord.Order2)) //Opening maker order already placed, but no taker, retrieve
            {
                ...
                {
                    msg = "Opening maker get: " + set.MakerOpeningOrder.ToString();
                }
        ...
        private void Message(string message, int runId)
        {
            if (runId > 0)
            {
                OnMessage?.Invoke(this, "<MakerTakerLeft><" + runId.ToString() + "> " + message);
            }
            else
            {
                OnMessage?.Invoke(this, "<MakerTakerLeft> " + message);
            }
        ...
        */
        ...
        private void MakerOrder_OnOrderAmended(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(fpx.OldOrderId) && fpx.OldOrderId != fpx.OrderID)
            {
                ...
                if (set != null)
                {
                    if (set.MakerOpeningOrder != null && set.MakerOpeningOrder.OrderID == fpx.OrderID)
                    {
                        set.RunRecord.Order1 = fpx.OrderID;
                    }
                    else if (set.MakerClosingOrderLeft != null && set.MakerClosingOrderLeft.OrderID == fpx.OrderID)
                    {
        ...
        internal class MakerTakerSet
        {
            ...
            public IApi Maker;
            public IApi Taker;
            public Timer TimerSettlement;    //time until settlement
            public Timer TimerReset;         //timeer for next run after settlement. Set after closing
            ...
            public async Task<bool> Process()
            {
                ...
                lock (syncLock)
                {
                    ...
                    if (bidPriceTotal > 0.0M)
                    {
                        ...
                        if (margin <= RunRecord.Num6.GetValueOrDefault())
                        {
                            ...
                            {                                
                                msg = "Cancelling left closing maker " + MakerClosingOrderLeft.OrderID;
                                Message(msg);
                                //MakerClosingOrderLeft.Stop("Close by Taker-Taker");
                                //resCancel = OrderManager.Instance.CancelOrderAsync(Maker, MakerClosingOrderLeft.OrderID).Result;
                ...
                if (processed)
                {
                    if (!string.IsNullOrEmpty(RunRecord.Order4) || !string.IsNullOrEmpty(RunRecord.Order6))
                    {
                        ...
                        if (RunRecord.Num12.GetValueOrDefault() == 0.0M && !string.IsNullOrEmpty(RunRecord.Order4)) //taker closing is empty
                        {
                            if (RunRecord.Text5.ToLower() == "buy")
                            {
                        ...
                        if (RunRecord.Num11.GetValueOrDefault() == 0.0M && !string.IsNullOrEmpty(RunRecord.Order6))
                        {
                            if (RunRecord.Text5.ToLower() == "buy")
                            {
